...
>
> **v3.1** · YOLO-World（開放詞彙偵測）× CLIP（跨模態視覺語意）二階段架構
...
│   ├── runtime/           # hub.py 狀態中樞 · settings.py 熱調參
│   │                      # vocabulary.py 詞彙 · lexicon.py 中文辭典 · personal.py 個人物品
│   └── api/               # app.py 工廠 · routes_stream/data/personal/system.py
...
```powershell
python -X utf8 tests\test_backend_api.py   # 30 項全端點回歸（搜尋/辭典/物品/串流/WS…）
...
- **側欄出現「後端版本不一致」警告**：後端還在跑舊程式碼，關閉後重跑 `start.ps1`。
...
- **規劃中**：主動通知（規則引擎＋Windows toast/Telegram）、出門模式（一鍵檢查所有個人物品）、加強搜尋模式、物品身分跨期持續、移動時間軸。

> **商業化注意**：ultralytics 為 AGPL-3.0 授權，商用需企業授權或更換偵測器；CLIP 為 MIT。

---

CtrlF v3.1 · 所有影像皆在本機運算，不上傳雲端
