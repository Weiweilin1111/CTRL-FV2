...
TIMEOUT = 2.5
UI_VERSION = "3.1.0"  # 必須與後端 ctrlf.__version__ 一致，否則提示重啟
...
with st.sidebar:
    ...
    for tag in fixed_tags:
        ...
        c1.markdown(f"<div style='padding-top:6px'>⭐ {tag}</div>", unsafe_allow_html=True)
    ...
    with st.expander("中文同義詞辭典"):
        ...
        for k, v in (al.get("custom") or {}).items():
            ...
            rc1.markdown(f"<div style='padding-top:6px'>{k} → {v}</div>", unsafe_allow_html=True)
...
tab_live, tab_manage = st.tabs(["即時尋物", "物品註冊管理"])
    ...
    def results_panel() -> None:
        ...
        if tracks:
            st.markdown(f'<span class="badge green">就在畫面裡！鎖定 {len(tracks)} 個目標</span>',
                        unsafe_allow_html=True)
            _cards_grid([
                f'<div class="t">{t["display"]}</div>'
                f'<div class="s">{loc_zh(t["location"])} · '
        ...
        if hist:
            ...
            _cards_grid([
                ...
                f'<div class="t">{r["display"]}</div>'
                f'<div class="s">{loc_zh(r["location"])} · 🕒 {r["last_seen"]}<br>'
        ...
        if not tracks and not hist:
            ...
            if sem.get("mode") == "visual" and results:
                ...
                _cards_grid([
                    f'<div class="row">{_thumb_html(r.get("thumb"))}<div>'
                    f'<div class="t">{r["display"]} '
                    f'<span class="badge blue">相似 {r["score"]:.0%}</span></div>'
                    f'<div class="s">{loc_zh(r["location"])} · 🕒 {r["last_seen"]}'
                    f'</div></div></div>'
                    for r in results
                ])
                ...
                _card('<div class="t">還沒有任何紀錄</div>'
                      f'<div class="s">相機正在持續為你掃描「{query}」。'
    ...
    def personal_panel() -> None:
        ...
        for i, it in enumerate(items):
            with cols[i % len(cols)]:
                if it.get("last_seen"):
                    status = (
                        f'{loc_zh(it.get("last_location"))} · 🕒 {it["last_seen"]}'
